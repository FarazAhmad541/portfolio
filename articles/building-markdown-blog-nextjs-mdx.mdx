---
title: Create Your Own Blog Using Next.js and MDX
metaDescription: Learn how to create a blog using Next.js and MDX. We will cover the basics of using MDX, how to create a custom component, and how to use MDX with a remote source.
createdAt: 2023-01-02
keywords: ['nextjs', 'mdx', 'markdown', 'syntax highlighting', 'code highlighting', 'next-mdx-remote', 'MDXRemote']
---

# Create Your Own Blog Using Next.js and MDX
In this article, we will learn how to create a blog using Next.js and MDX.
We will cover the basics of using MDX, how to create a custom components,
...
Markdoc to format markdown files are CPU intensive, and if the markdown file is large
then rendering all that on client side can increase page load timees.\
Using Next.js these files can be rendered on the server at build time and then prerendered
HTML can be sent to the client, that can significantly increase page load speeds.</InfoBlock>
...
```ts showLineNumbers {10} title="./tailwind.config.ts"
...
If your markdown files are hosted on some external source like GitHub Pages, database or CMS,
...
```
This package provides us a function *`MDXRemote`*. This package is different from *`next-mdx-remote/rsc`* package mentioned in the Next.js docs, but has better comprehensive documentation and is more actively maintained.
